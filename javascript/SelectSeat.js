...
document.addEventListener("DOMContentLoaded", function () {
  ...
  seatCountEl = document.getElementById("seat-count");

  if (!gridA || !gridB || !seatCountEl) {
  ...
  selectCompletion.addEventListener("click", () => {
    if (selectedSeatNames.length < 1) {
      ...
      sessionStorage.setItem("Seatprice", selectedSeatNames.length * SEAT_PRICE);
...
function getRandomSeatStatus() {
  return Math.random() < DISABLED_RATIO ? "disabled" : "selectable";
}
...
function createSeat(targetGrid, areaName) {
  for (let i = 0; i < TOTAL_SEATS; i++) {
    ...
    seat.classList.add(getRandomSeatStatus());
...
function handleSeatClick(seat) {
  seat.addEventListener("click", () => {
    ...
    if (seat.classList.contains("selectable")) {
      if (selectedSeatNames.length < MAX_SEAT_COUNT) {
        seat.classList.remove("selectable");
        seat.classList.add("selected");
        selectedSeatNames.push(seatName);
        seatCountEl.textContent = `총 ${selectedSeatNames.length}석`;
      } else {
        alert(`최대 ${MAX_SEAT_COUNT}매까지 가능합니다`);
      }
    }
